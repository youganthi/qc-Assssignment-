...
p_control <- plot_ly(control_chart_data, type = "scatter", mode = "lines+markers") %>%
  ...
  add_trace(y = ~Mean, name = "Mean", mode = "lines", line = list(color = "black", dash = "dash"), showlegend = TRUE) %>%
  add_trace(y = ~UCL, name = "UCL (3σ)", mode = "lines", line = list(color = "#D55E00", dash = "dot"), showlegend = TRUE) %>%
  add_trace(y = ~LCL, name = "LCL (3σ)", mode = "lines", line = list(color = "#D55E00", dash = "dot"), showlegend = TRUE) %>%
  layout(
...
htmlwidgets::saveWidget(p_control, paste0("media/plots/", plot_name_control, ".html"), selfcontained = TRUE)
